 ...
 *
 * Ce script :
 * 1. Récupère les nouveaux leads depuis Supabase (leads où notified_at IS NULL)
 * 2. Envoie le guide PDF par email
 * 3. Marque le lead comme notifié dans Supabase
 * 4. Ajoute une ligne dans le Google Sheet avec les dates
 *
 * Installation :
 * 1. Dans Google Sheets → Extensions > Apps Script
 * 2. Coller ce code
 * 3. Configurer les propriétés du script (voir ci-dessous)
 * 4. Créer un déclencheur temporel (toutes les heures)
 */

// ========== CONFIGURATION ==========
const SUPABASE_URL = 'https://tzdypqpexlwugehsscec.supabase.co';
// La clé sera lue depuis les propriétés du script (plus sûr)
...
function sendGuideToLeads() {
  ...
  // Récupérer les leads non notifiés depuis Supabase
...
// ========== SUIVI (FOLLOW-UP) ==========
/**
 * À exécuter séparément (trigger tous les 2 jours) pour envoyer les emails de suivi
 */
function sendFollowUps() {
  ...
  rows.forEach((row, index) => {
    const email = row[0]; // colonne A
    const createdAt = row[2] ? new Date(row[2]) : null; // colonne C
    const notifiedAt = row[3] ? new Date(row[3]) : null; // colonne D
    const followUp1Sent = row[4]; // colonne E
    const followUp2Sent = row[5]; // colonne F
    
    if (!email || !notifiedAt) return;
    
    const daysSinceNotified = (now - notifiedAt) / (1000 * 60 * 60 * 24);
    
    // Follow-up 1 : 2 jours après l'envoi du guide
    if (!followUp1Sent && daysSinceNotified >= 2) {
      sendFollowUpEmail(email, 1, supabaseKey);
      sheet.getRange(index + 2, 5).setValue(now.toISOString()); // colonne E
    }
    
    // Follow-up 2 : 7 jours après l'envoi du guide
    if (!followUp2Sent && daysSinceNotified >= 7) {
      sendFollowUpEmail(email, 2, supabaseKey);
      sheet.getRange(index + 2, 6).setValue(now.toISOString()); // colonne F
    }
  });
}
...
function sendFollowUpEmail(email, sequence, supabaseKey) {
  ...
  if (sequence === 1) {
    body = `
      ...
      <p>Des questions ? N'hésitez pas à me répondre directement à cet email.</p>
    ...
    body = `
      ...
      <p>En ce moment, profitez de <strong>-20% sur votre première analyse</strong> avec le code <strong>WELCOME20</strong>.</p>
      <p><a href="https://tennis-breakdown-pwaf31cr9-sarfaoui4s-projects.vercel.app/pricing" style="background-color: #ea580c; color: white; padding: 10px 20px; text-decoration: none; border-radius: 6px;">Voir les tarifs</a></p>
